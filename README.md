...
****<a class="link-gray" href="https://github.com/SkriptLang/Skript/releases">Skript</a>是一款 <a class="link-gray" href="https://www.minecraft.net/zh-hant/">Minecraft</a> 的腳本插件，其主要功用為提供給不會寫程式語言的人，讓他們能撰寫屬於自己的腳本，讓他們達成想讓伺服器做的任何事情****
...
# 8.Functions
  ...
  ```diff
  function 函式名稱([變數名稱: 資料型態] , [變數名稱: 資料型態] , ......) :: [回傳的資料型態]:
    #寫在大函式名稱()內的我們稱之為引數，將外部資料引入函式內部的意思。
  ...
  #宣告一個名為 getPlayerLoc()的函式，其引數為player，導入後以{_p}作為區域變數在函式內使用，其資料回傳型態為location(位置)。
    ...
    return {_loc} #回傳{_loc}內的value

  #使用函式
  command /loc [<player>]: #註冊一個指令: /loc <玩家名稱>
    trigger: #這是註冊指令必寫語句，意思就是執行這指令會觸發什麼Effects

      set {_targetLoc} to getPlayerLoc(arg) 
      #設定 {_targetLoc} 為 getPlayerLoc(arg) 的回傳值；其中 getPlayerLoc(arg) 的 arg 就是 /loc <玩家名稱> ，<>內的玩家名稱，例如: /loc NUTT1101 ，那 arg 等於 NUTT1101，資料型態為player。
